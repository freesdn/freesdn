...
"""
FreeSDN — Gateway OPNsense Cron endpoint.

URL layout::

    GET   /api/v1/gateway-opnsense-cron/{controller_id}/jobs
    POST  /api/v1/gateway-opnsense-cron/{controller_id}/changes/{feature}
    GET   /api/v1/gateway-opnsense-cron/{controller_id}/changes
...
Stage endpoint locks ``feature`` to ``opnsense.cron.*`` so a caller
with ``firewall:write`` can't smuggle a non-cron feature through this
URL. Reads inherit ``firewall:read`` because OPNsense IS the firewall
— scheduled tasks are a sub-feature of that surface.
"""
...
router = APIRouter(
    prefix="/gateway-opnsense-cron",
    tags=["gateway-opnsense-cron"],
)
...
async def list_jobs(
    ...
    svc = GatewayOpnsenseCronService(session)
...
async def stage_opnsense_cron_change(
    controller_id: UUID,
    feature: str,
    ...
    if not feature.startswith("opnsense.cron."):
        raise HTTPException(
            400,
            detail=("OPNsense cron endpoint only accepts opnsense.cron.* features"),
        )
    ...
    if operation in ("update", "delete"):
        if not body.target_id:
            raise HTTPException(
                400,
                detail=("OPNsense cron update/delete requires target_id"),
            )
...
async def list_pending_opnsense_cron(
    controller_id: UUID,
    user: Annotated[CurrentUser, Depends(require_permissions("firewall:read"))],
    session: Annotated[AsyncSession, Depends(get_session)],
