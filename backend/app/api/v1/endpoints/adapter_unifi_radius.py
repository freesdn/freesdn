...
"""FreeSDN — Gateway UniFi RADIUS endpoint (built-in RADIUS user accounts)."""
...
router = APIRouter(prefix="/gateway-unifi-radius", tags=["gateway-unifi-radius"])
...
@router.get("/{controller_id}/sites/{site}/users")
async def list_radius_users(controller_id: UUID, site: str, user: _READ, session: _Session) -> Any:
    return await GatewayUniFiRadiusService(session).list_users(
...
async def stage_radius_change(
    ...
    if not feature.startswith("unifi.radius."):
        raise HTTPException(400, detail="This endpoint only accepts unifi.radius.* features")
    change = await GatewayUniFiRadiusService(session).stage_change(
        feature=feature,
        operation=operation,
