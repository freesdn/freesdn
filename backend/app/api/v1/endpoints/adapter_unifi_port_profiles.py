...
"""FreeSDN — Gateway UniFi Port Profiles endpoint (switch port profiles)."""
...
router = APIRouter(prefix="/gateway-unifi-port-profiles", tags=["gateway-unifi-port-profiles"])
...
@router.get("/{controller_id}/sites/{site}/profiles")
async def list_port_profiles(controller_id: UUID, site: str, user: _READ, session: _Session) -> Any:
    return await GatewayUniFiPortProfilesService(session).list_port_profiles(
...
async def stage_port_profile_change(
    controller_id: UUID,
    feature: str,
    ...
    if not feature.startswith("unifi.portprofiles."):
        raise HTTPException(400, detail="This endpoint only accepts unifi.portprofiles.* features")
    change = await GatewayUniFiPortProfilesService(session).stage_change(
        feature=feature,
        operation=operation,
...
async def list_pending_port_profiles(
    controller_id: UUID,
