...
"""FreeSDN — Gateway UniFi Radios endpoint (AP RF management)."""
...
router = APIRouter(prefix="/gateway-unifi-radios", tags=["gateway-unifi-radios"])
...
@router.get("/{controller_id}/sites/{site}/radios")
async def list_radios(controller_id: UUID, site: str, user: _READ, session: _Session) -> Any:
    return await GatewayUniFiRadiosService(session).list_radios(
...
async def stage_radio_change(
    ...
    if not feature.startswith("unifi.radios."):
        raise HTTPException(400, detail="This endpoint only accepts unifi.radios.* features")
    change = await GatewayUniFiRadiosService(session).stage_change(
        feature=feature,
        operation=operation,
