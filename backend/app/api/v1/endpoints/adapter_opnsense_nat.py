...
"""
FreeSDN — Gateway OPNsense NAT endpoint.

URL layout::

    GET   /api/v1/gateway-opnsense-nat/{controller_id}/source-rules
    GET   /api/v1/gateway-opnsense-nat/{controller_id}/port-forwards
    POST  /api/v1/gateway-opnsense-nat/{controller_id}/changes/{feature}
    GET   /api/v1/gateway-opnsense-nat/{controller_id}/changes
...
Stage endpoint locks ``feature`` to ``opnsense.nat.*`` so a caller
with ``firewall:write`` can't smuggle a non-NAT feature through this
URL.
"""
...
router = APIRouter(
    prefix="/gateway-opnsense-nat",
    tags=["gateway-opnsense-nat"],
)


@router.get("/{controller_id}/source-rules")
async def list_source_rules(
    controller_id: UUID,
    user: Annotated[CurrentUser, Depends(require_permissions("firewall:read"))],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> Any:
    svc = GatewayOpnsenseNatService(session)
    return await svc.list_source_rules(controller_id, user.organization_id)
...
async def list_port_forwards(
    ...
    svc = GatewayOpnsenseNatService(session)
    return await svc.list_port_forwards(controller_id, user.organization_id)
...
async def stage_opnsense_nat_change(
    controller_id: UUID,
    feature: str,
    ...
    if not feature.startswith("opnsense.nat."):
        raise HTTPException(
            400,
            detail=("OPNsense NAT endpoint only accepts opnsense.nat.* features"),
        )
    ...
    if operation in ("update", "delete"):
        if not body.target_id:
            raise HTTPException(
                400,
                detail=("OPNsense NAT update/delete requires target_id"),
            )
...
async def list_pending_opnsense_nat(
    controller_id: UUID,
    user: Annotated[CurrentUser, Depends(require_permissions("firewall:read"))],
    session: Annotated[AsyncSession, Depends(get_session)],
