...
"""FreeSDN — Gateway UniFi WLAN Groups endpoint (WLAN groups)."""
...
router = APIRouter(prefix="/gateway-unifi-wlan-groups", tags=["gateway-unifi-wlan-groups"])
...
async def list_wlan_groups(controller_id: UUID, site: str, user: _READ, session: _Session) -> Any:
    return await GatewayUniFiWlanGroupsService(session).list_wlan_groups(
...
async def stage_wlan_group_change(
    controller_id: UUID,
    feature: str,
    ...
    if not feature.startswith("unifi.wlangroups."):
        raise HTTPException(400, detail="This endpoint only accepts unifi.wlangroups.* features")
    change = await GatewayUniFiWlanGroupsService(session).stage_change(
        feature=feature,
        operation=operation,
...
async def list_pending_wlan_groups(
    controller_id: UUID,
