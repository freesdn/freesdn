...
"""FreeSDN — Gateway UniFi Routing endpoint (static routes)."""
...
router = APIRouter(prefix="/gateway-unifi-routing", tags=["gateway-unifi-routing"])
...
async def list_routing(controller_id: UUID, site: str, user: _READ, session: _Session) -> Any:
    return await GatewayUniFiRoutingService(session).list_routing(
...
async def stage_routing_change(
    controller_id: UUID,
    feature: str,
    ...
    if not feature.startswith("unifi.routing."):
        raise HTTPException(400, detail="This endpoint only accepts unifi.routing.* features")
    change = await GatewayUniFiRoutingService(session).stage_change(
        feature=feature,
        operation=operation,
